...
function wkhtmltopdf(input, options, callback) {
    ...
    args.push(isUrl ? quote(input.input) : '-');    // stdin if HTML given directly
    ...
    // write input.input to stdin if it isn't a url
    if (!isUrl) {
        if (isStream(input.input)) {
            input.input.pipe(child.stdin);
        } else {
